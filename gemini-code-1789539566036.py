...
with st.container():
  ...
  col1, col_arrow1, col2, col_arrow2, col3 = st.columns(
      [1.2, 0.15, 1.3, 0.15, 1.6]
  )
  ...
  with col2:
    ...
    st.markdown(
        "- 진로, 탐구, 멘토링, 독서 등 (주로) 학년부 중심의 교육활동[cite: 3]"
    )
    st.markdown(
    ...
    st.markdown(
        "- *실제 필요성은 있으나 효과성이 없거나, 하는지 아무도 모름*[cite: 3]"
    )
  ...
  with col3:
    ...
    st.markdown(
        "- **불가능한 것 / 필요성이 불분명한 것** $\Rightarrow$ **걷어내기**"
        "[cite: 3]"
    )
    ...
    st.markdown(
        "- **해야 하고 (제대로 기획하면) 할 수 있는 것** $\Rightarrow$ **맥락화**"
