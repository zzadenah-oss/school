...
with st.container():
  ...
  with col4:
    st.markdown(
        ...
        " **걷어내기**<br><span style='font-size:12px;"
        " color:#555;'>(행정적으로 대응, '잘하려'하지 않기)</span>",
        unsafe_allow_html=True,
    )
...
with st.container():
  ...
  with col2:
    ...
    with sub2:
      st.markdown("**접근 및 대응(\"업무보다, 수업보다 대화\")**")
      st.markdown("- **전제:** 개별 교사 차원의 문제를 개인 책임감에 의존하지 않음")
      st.markdown(
          "- **대응 ①:** 시간표 내 일상적/정기적/안정적 대화 구조 확립"
      )
      st.markdown(
          "- **대응 ②:** 교사가 교육활동 기획의 주체로 (권한은 실무자에게,"
          " 책임과 부담은 함께 -> 일하는 사람이 외롭지 않도록)"
      )
