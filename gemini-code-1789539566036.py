...
with st.container():
  ...
  col1, arrow1, col2, arrow2, col3, arrow3, col4 = st.columns(
      [1.1, 0.1, 1.2, 0.1, 1.0, 0.1, 1.6]
  )

  with col1:
    st.markdown("### 1. 형식적인 교육활동")
    st.markdown("- 교사의 노동에 비해 교육적 효과가 미미 또는 유명무실")
    st.markdown("- 행정적으로 잘하려 하지 않기")
  ...
  with col2:
    ...
    st.markdown("- 진로, 탐구, 멘토링, 독서 등 (주로) 학년부 중심의 교육활동")
    st.markdown("- 정규교과 수업 밖에서 이루어지는 교과/비교과 차원의 교육활동")
    ...
    st.markdown("**해야 하니까 하는 것(들)**")
    st.markdown("- 각종 예방교육, 사회정서교육, 민주시민교육 등")
    st.markdown(
        "- *실제 필요성은 있으나 효과성이 없거나, 하는지 아무도 모름*"
    )
...
with st.container():
  ...
  with col1:
    st.markdown("### 2. 교사의 피로와 무력감")
    st.markdown("- 관계와 맥락은 없고 일만 남은 소모적 전쟁 상태")
...
with st.container():
  ...
  with col1:
    st.markdown("### 3. 규범의 공백")
    st.markdown("- 금지와 제한이 사라진 공백")
...
st.markdown(
    ...
    <div style="background-color: #FFF5F5; border: 2px solid #FEB2B2; border-radius: 8px; padding: 20px; margin-top: 25px; text-align: center;">
        <h3 style="color: #9B2C2C; margin-bottom: 15px;">🎯 최종 지향점: 교사의 성장</h3>
    """,
    unsafe_allow_html=True,
)
